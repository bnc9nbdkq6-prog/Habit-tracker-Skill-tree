...
export function layout(nodes) {
  ...
  function measure(n, depth) {
    ...
    for (const child of nodes.filter((x) => x.parentId === n.id))
      measure(child, depth + 1);
  }
  roots.forEach((n) => measure(n, 1));
  ...
  function place(n, a, b, depth) {
    const angle = (a + b) / 2;
    // Start with weekly milestones near the centre; long-term visions form
    // the outermost destination at the end of each branch.
    const r = 290 * (maxDepth - depth + 1);
