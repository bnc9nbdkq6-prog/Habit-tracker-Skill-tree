  ...
  useEffect(()=>{
    ...
    stSaveRef.current=setTimeout(async()=>{
      try{
        ...
        if(stProjs.length){
          await supabase.from('st_projects').upsert(stProjs.map(p=>({id:p.id,user_id:userId,domain_id:p.domainId,name:p.name,color:p.color,description:p.description||''})),{onConflict:'id'})
        ...
        <circle r={r+5} fill="transparent"
          ...
          onTouchStart={e=>{
            ...
            if(editMode){
              ...
              const t=e.touches[0]
              const rect=svgRef.current?.getBoundingClientRect()||{left:0,top:0}
              const{wx,wy}=toWorld(t.clientX-rect.left,t.clientY-rect.top,tf)
              dragRef.current={active:true,nid:n.id,ox:n.x-wx,oy:n.y-wy}
            }else startHold(n.id)
          }}
          onTouchEnd={e=>{e.stopPropagation();stopHold();dragRef.current.active=false}}
        />
        {/* Edit: + child */}
        {editMode&&(
