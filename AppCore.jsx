...
function SkillTree({onBack,userId}){
  const [stDomains,setStDomains]=useState(()=>{try{const s=localStorage.getItem('st_domains');return s?JSON.parse(s):INIT_STD}catch{return INIT_STD}})
  const [stProjs,setStProjs]=useState(()=>{try{const s=localStorage.getItem('st_projs');return s?JSON.parse(s):INIT_STP}catch{return INIT_STP}})
  const [stNodes,setStNodes]=useState(()=>{try{const s=localStorage.getItem('st_nodes');return s?JSON.parse(s):INIT_STN}catch{return INIT_STN}})
  const [stLoaded,setStLoaded]=useState(false)

  // Laad Skill Tree data uit Supabase
  useEffect(()=>{
    if(!userId)return
    const load=async()=>{
      try{
        const[{data:doms,error:e1},{data:projs},{data:nodes},{data:lpos}]=await Promise.all([
        ...
        if(e1)console.error('[ST load error]',e1)
        if(doms?.length)setStDomains(doms.map(r=>({id:r.id,name:r.name,color:r.color,sort_order:r.sort_order})))
        if(projs?.length)setStProjs(projs.map(r=>({id:r.id,domainId:r.domain_id,name:r.name,color:r.color,description:r.description||''})))
        if(nodes?.length)setStNodes(nodes.map(r=>({
          id:r.id,projectId:r.project_id,title:r.title,description:r.description||'',
          achieveDesc:r.achieve_desc||'',status:r.status||'locked',parentId:r.parent_id||null,
          x:r.x||0,y:r.y||0,isRoot:r.is_root||false,dTarget:r.d_target||null,dHard:r.d_hard||null,
          refl:r.refl||{wellDone:'',obstacles:''}
        })))
        if(lpos?.positions)setLabelPos_(lpos.positions)
      }catch(e){console.error('[ST load exception]',e)
      }finally{setStLoaded(true)}
    }
    load()
    // Realtime + polling voor sync tussen apparaten
    const ch=supabase.channel('st-rt-'+userId)
  ...
  // Sla Skill Tree op in localStorage EN Supabase
  useEffect(()=>{
    try{localStorage.setItem('st_domains',JSON.stringify(stDomains))}catch{}
    if(!userId||!stLoaded)return
    const rows=stDomains.map((d,i)=>({id:d.id,user_id:userId,name:d.name,color:d.color,sort_order:i}))
    supabase.from('st_domains').upsert(rows,{onConflict:'id'})
  },[stDomains,userId,stLoaded])
  useEffect(()=>{
    try{localStorage.setItem('st_projs',JSON.stringify(stProjs))}catch{}
    if(!userId||!stLoaded)return
    const rows=stProjs.map(p=>({id:p.id,user_id:userId,domain_id:p.domainId,name:p.name,color:p.color,description:p.description||''}))
    supabase.from('st_projects').upsert(rows,{onConflict:'id'})
  },[stProjs,userId,stLoaded])
  useEffect(()=>{
    try{localStorage.setItem('st_nodes',JSON.stringify(stNodes))}catch{}
    if(!userId||!stLoaded)return
    const rows=stNodes.map(n=>({
      id:n.id,user_id:userId,project_id:n.projectId||null,title:n.title||'',
      description:n.description||'',achieve_desc:n.achieveDesc||'',status:n.status||'locked',
      parent_id:n.parentId||null,x:n.x||0,y:n.y||0,is_root:n.isRoot||false,
      d_target:n.dTarget||null,d_hard:n.dHard||null,refl:n.refl||{wellDone:'',obstacles:''}
    }))
    supabase.from('st_nodes').upsert(rows,{onConflict:'id'})
  },[stNodes,userId,stLoaded])
  const [labelPos,setLabelPos_]=useState(()=>{try{const s=localStorage.getItem('st_label_pos');return s?JSON.parse(s):{}}catch{return{}}})
  useEffect(()=>{
    try{localStorage.setItem('st_label_pos',JSON.stringify(labelPos))}catch{}
    if(!userId||!stLoaded)return
    supabase.from('st_label_positions').upsert({user_id:userId,positions:labelPos},{onConflict:'user_id'})
  },[labelPos,userId,stLoaded])
...
function HabitTracker({onBack,userId}) {
  ...
  // Laad Habit Tracker data uit Supabase bij opstarten
  useEffect(()=>{
    if(!userId)return
    const load=async()=>{
      ...
      if(s?.value){const today=new Date().toDateString();setState(prev=>({...DEFAULT_STATE,...s.value,todayDate:s.value.todayDate===today?s.value.todayDate:today,todaySubmitted:s.value.todayDate===today?s.value.todaySubmitted:false}))}
      if(cv?.value&&Array.isArray(cv.value))setCheckValues(cv.value)
    }
    load().then(()=>setHtLoaded(true)).catch(()=>setHtLoaded(true))
    // Realtime + polling voor sync
    const ch=supabase.channel('ht-rt-'+userId)
  ...
  useEffect(() => {
    ...
    supabase.from('user_data').upsert({user_id:userId,key:'habit_state',value:state,updated_at:new Date().toISOString()},{onConflict:'user_id,key'})
      .then(({error})=>{if(error)console.error('[HT state save]',error)})
  }, [state,userId,htLoaded]);
  useEffect(() => {
    try { localStorage.setItem("discipline-engine-checks", JSON.stringify(checkValues)); } catch {}
    if(!userId)return
    supabase.from('user_data').upsert({user_id:userId,key:'habit_checks',value:checkValues,updated_at:new Date().toISOString()},{onConflict:'user_id,key'})
  }, [checkValues,userId]);
  useEffect(() => {
    try {
      const saved = localStorage.getItem("discipline-engine-checks");
