...
function STDomainManager({domains,stProjs,stNodes,setDomains,setStProjs,setStNodes,onClose}){
  ...
  const addProject=(d)=>{
    ...
    const newP={id:pid,domainId:d.id,name:newPName.trim(),color:newPColor,description:'',why:newPWhy.trim()}
    setStProjs(ps=>[...ps,newP])
    ...
    const load=async()=>{
      try{
        ...
        if(!stPending.current){
          ...
          if(projs?.length){const m=projs.map(r=>({id:r.id,domainId:r.domain_id,name:r.name,color:r.color,description:r.description||'',why:r.why||''}));setStProjs(m);try{localStorage.setItem('st_projs_'+userId,JSON.stringify(m))}catch{}}
  ...
  useEffect(()=>{
    ...
    stSaveRef.current=setTimeout(async()=>{
      try{
        ...
        if(stProjs.length){
          await supabase.from('st_projects').upsert(stProjs.map(p=>({id:p.id,user_id:userId,domain_id:p.domainId,name:p.name,color:p.color,description:p.description||'',why:p.why||''})),{onConflict:'id'})
      ...
      {showTrophies&&(
        <div style={{position:'fixed',inset:0,zIndex:600,display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:40}}>
          ...
          <div style={{position:'relative',background:'#0c0c0c',width:'100%',maxWidth:'640px',maxHeight:'80vh',borderRadius:'6px',border:`1px solid ${BR}`,display:'flex',flexDirection:'column',margin:'0 12px'}}>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'16px 20px'}}>
              {stProjs.filter(p=>{
                const nodes=stNodes.filter(n=>n.projectId===p.id)
