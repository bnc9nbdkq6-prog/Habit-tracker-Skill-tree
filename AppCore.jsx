...
export default function App(){
      ...
      try{
        ...
        if(tRows?.length){
          setTasks(tRows.map(r=>({
            ...
            completed:r.completed,subtasks:r.subtasks||[],
            recurrence:r.recurrence,scheduledTime:r.scheduled_time
          })))
        }
        if(pRows?.length){
          setProjs(pRows.map(r=>({
  ...
  // Save tasks to Supabase on change (debounced)
  ...
  // Save projects to Supabase on change
  useEffect(()=>{
    if(!userId)return
    ...
    supabase.from('projects').upsert(rows,{onConflict:'id'})
  },[projs,userId])
  ...
  if(screen==='skilltree')return <SkillTree onBack={()=>setScreen('projects')}/>
  if(screen==='habittracker')return <HabitTracker onBack={()=>setScreen('projects')}/>
  return(
...
function SkillTree({onBack}){
  ...
  useEffect(()=>{try{localStorage.setItem('st_domains',JSON.stringify(stDomains))}catch{}},[stDomains])
  useEffect(()=>{try{localStorage.setItem('st_projs',JSON.stringify(stProjs))}catch{}},[stProjs])
  useEffect(()=>{try{localStorage.setItem('st_nodes',JSON.stringify(stNodes))}catch{}},[stNodes])
  const [labelPos,setLabelPos_]=useState(()=>{try{const s=localStorage.getItem('st_label_pos');return s?JSON.parse(s):{}}catch{return{}}})
  useEffect(()=>{try{localStorage.setItem('st_label_pos',JSON.stringify(labelPos))}catch{}},[labelPos])
