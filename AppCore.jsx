...
function SkillTree({onBack,userId}){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      if(projs?.length)setStProjs(projs.map(r=>({id:r.id,domain_id:r.domain_id,name:r.name,color:r.color,description:r.description||''})))
      if(nodes?.length)setStNodes(nodes.map(r=>({
        id:r.id,project_id:r.project_id,title:r.title,description:r.description||'',
        achieve_desc:r.achieve_desc||'',status:r.status||'locked',parent_id:r.parent_id||null,
        x:r.x||0,y:r.y||0,is_root:r.is_root||false,d_target:r.d_target||null,d_hard:r.d_hard||null,
  ...
  useEffect(()=>{
    ...
    if(!userId||!stLoaded)return
    const rows=stProjs.map(p=>({id:p.id,user_id:userId,domain_id:p.domain_id,name:p.name,color:p.color,description:p.description||''}))
  ...
  useEffect(()=>{
    ...
    const rows=stNodes.map(n=>({
      id:n.id,user_id:userId,project_id:n.project_id,title:n.title||'',
      description:n.description||'',achieve_desc:n.achieve_desc||'',status:n.status||'locked',
      parent_id:n.parent_id||null,x:n.x||0,y:n.y||0,is_root:n.is_root||false,
      d_target:n.d_target||null,d_hard:n.d_hard||null,refl:n.refl||{wellDone:'',obstacles:''}
    }))
